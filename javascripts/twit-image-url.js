...
SpazImageURL.prototype.findServiceUrlsInString = function(str) {
	...
	for (key in this.apis) {
		...
		// TM.helpers.debug(thisapi.url_regex);
		while( (re_matches = thisapi.url_regex.exec(str.strip())) != null) {
			// TM.helpers.debug(re_matches);
			matches[key] = re_matches;
			num_matches++;
		}
	}
	//TM.helpers.debug('num_matches:'+num_matches);
	//TM.helpers.debug(matches);	
	if (num_matches > 0) {
		return matches;
	} else {
		return null;
...
SpazImageURL.prototype.getThumbsForMatches = function(matches) {
	...
	for (service in matches) {
		Mojo.Log.info('SERVICE:'+service);
		api = this.getAPI(service);
		urls = matches[service]; // an array
		Mojo.Log.info("URLS:"+urls);
		Mojo.Log.info('Length of URLs', urls.length);
		thumburls[urls[0]] = api.getThumbnailUrl(urls[1]);

		num_urls++;
	}

	Mojo.Log.info('num_urls:'+num_urls);
	Mojo.Log.info('Thumbnail urls', thumburls);	
...
SpazImageURL.prototype.getImagesForMatches = function(matches) {
	...
	for (service in matches) {
		// TM.helpers.debug('SERVICE:'+service);
		api = this.getAPI(service);
		urls = matches[service]; // an array
		// TM.helpers.debug("URLS:"+urls);
		imageurls[urls[0]] = api.getImageUrl(urls[1]);
		num_urls++;
	}

	// TM.helpers.debug('num_urls:'+num_urls);
	// TM.helpers.debug(imageurls);	
